...
const findColumn = (columnList: IQueryColumnOut[]): [string, Set<string>, Set<string>] => {
  ...
  const normal = columnList
    ...
    .map((p) => {
      ...
      if (
        gqlType !== 'String' &&
        gqlType !== 'GraphQLGeometryPoint' &&
        gqlType !== 'GraphQLJSONObject'
      ) {
        importGqlType.add(gqlType);
      } else if (gqlType === 'GraphQLGeometryPoint') {
        importOtherType.add(
          "import { GraphQLGeometryPoint } from 'src/utils/object-type/graphql-geometry-point';"
        );
...
const modelTemplate = ({
  ...
  const txt = `import { ObjectType, Field, ${gqlTypeImport} } from '@nestjs/graphql';
import { BaseEntity } from 'src/utils/base-entity';${txtImport}
